    ...
    >
      ...
      >
        <el-descriptions-item label="日期">
          {{ formatDate(detailData.reportDate) }}
        ...
        <el-descriptions-item label="日报内容">
    ...
    >
      ...
      >
        <el-descriptions-item label="学生姓名">
          {{ reviewRow.studentName }}
        </el-descriptions-item>
        <el-descriptions-item label="日期">
          {{ formatDate(reviewRow.reportDate) }}
        </el-descriptions-item>
        <el-descriptions-item label="日报内容">
          <div style="white-space: pre-wrap">
            {{ reviewRow.content }}
        ...
        <el-descriptions-item label="照片">
          <div
            v-if="reviewRow.photos && reviewRow.photos.length"
            class="detail-photos"
          >
            <el-image
              v-for="(url, index) in reviewRow.photos"
              :key="index"
              :src="url"
              :preview-src-list="reviewRow.photos"
              :initial-index="index"
              fit="cover"
              style="width: 100px; height: 100px; margin-right: 8px"
            />
        ...
        <el-descriptions-item label="附件">
          <template v-if="reviewRow.attachments && reviewRow.attachments.length">
            <el-link
              v-for="(file, index) in reviewRow.attachments"
              :key="index"
              :href="file.url"
              target="_blank"
              type="primary"
...
function disableFutureDate(date) {
  ...
  if (!editId.value) {
    const ds = `${date.getFullYear()}年${String(date.getMonth() + 1).padStart(2, '0')}月${String(date.getDate()).padStart(2, '0')}日`
    return tableData.value.some(r => r.reportDate === ds)
  }
  return false
}
...
async function loadPlanOptions() {
  try {
    const res = await request.get('/internship/plans/my-plans')
...
async function fetchData() {
  ...
  try {
    ...
    console.warn('获取日报列表失败', e.message)
  } finally {
    loading.value = false
  }
}
...
function handleAdd() {
  resetForm()
  dialogTitle.value = '填写日报'
  dialogVisible.value = true
}

function handleEdit(row) {
  resetForm()
  dialogTitle.value = '编辑日报'
  editId.value = row.id
...
function handlePhotoSuccess(response, uploadFile) {
  uploadFile.id = response.data?.filePath
...
async function handleSubmit() {
  ...
  if (!editId.value) {
    ...
    if (dup) {
      ElMessage.warning('该日期已存在日报，不可重复提交')
      return
    }
  }
  ...
  try {
    const submitData = {
      ...
      photoPaths: form.photoList.map(f => f.id || f.response?.data?.filePath).filter(Boolean).join(','),
      attachmentPaths: form.attachmentList.map(f => f.id || f.response?.data?.filePath).filter(Boolean).join(',')
    }
    if (editId.value) {
